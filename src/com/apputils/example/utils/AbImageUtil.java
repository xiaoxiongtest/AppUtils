...
import com.apputils.example.dtc.FDCT;
import com.apputils.example.http.bitmap.ImageCallBack;
...
public class AbImageUtil {
	...
	private static AbImageUtil abImageUtil;
	private BitmapUtils bitmapUtils;

	private AbImageUtil(Context context) {
		this.context = context;
		bitmapUtils = new BitmapUtils(context);
	}

	public static AbImageUtil getInstance(Context context) {
		if (abImageUtil == null) {
			abImageUtil = new AbImageUtil(context);
		}
		return abImageUtil;
	}

	public void getBitmapFromUrl(String imageUrl, final ImageCallBack imageCallBack) {
		if (bitmapUtils == null || context == null) {
			imageCallBack.onLoadFailed("bitmapUtils == null or context == null");
			return;
		}
		ImageView imageView = new ImageView(context);
		bitmapUtils.display(imageView, imageUrl, new BitmapLoadCallBack<ImageView>() {

			@Override
			public void onLoadCompleted(ImageView arg0, String arg1, Bitmap arg2, BitmapDisplayConfig arg3,
					BitmapLoadFrom arg4) {
				imageCallBack.onLoadCompleted(arg1, arg3, arg2, arg4);
			}

			@Override
			public void onLoadFailed(ImageView arg0, String arg1, Drawable arg2) {
				imageCallBack.onLoadFailed(arg1);
			}
	...
	public Bitmap revitionImageSize(String path, int size) {
		...
		try {
			...
			MLog.D(MLog.TAG_IMG, "revitionImageSize Error:" + e.getMessage());
		}
		return bitmap;
	}
	...
	public static Bitmap toRoundBitmap(Bitmap bitmap) {
		...
		int width = bitmap.getWidth();
		int height = bitmap.getHeight();
		return toRoundBitmap(bitmap, width <= height ? width : height);
	}

	public static Bitmap toRoundBitmap(Bitmap bitmap, int r) {
		if (bitmap == null) {
			return null;
		}
		int width = bitmap.getWidth();
		...
		roundPx = r / 2;
		float clipW = (width - r) / 2;
		left = clipW;
		right = width - clipW;
		float clipH = (height - r) / 2;
		top = clipH;
		bottom = height - clipH;
		width = r;
		height = r;
		...
		dst_right = r;
		dst_bottom = r;
		MLog.D(MLog.TAG_IMG, "top:" + top + ",left:" + left + ",buttom:" + bottom + ",right:" + right);
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					MLog.D(MLog.TAG_IMG, "Bitmap释放" + bitmap.toString());
					bitmap.recycle();
				}
			} catch (Exception e) {
			}
